...
class JollofDataArangoDB {
	...
	* findById( collectionName, id, params ) {
		const q = {};
		q[ this.idField ] = id;
		const cursor = yield this._db.collection(collectionName).firstExample(q);
		return cursor.next();
	}
	...
	* findOne( collectionName, criteria, params ) {
		const cursor = yield this._db.collection(collectionName).firstExample(this._processCriteria(criteria), params.opts)
		return cursor.next();
	}
