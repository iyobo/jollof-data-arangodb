...
class JollofDataArangoDB {
	...
	* delete( model ) {
		...
		const res = yield this._db.collection(model._collectionName).remove(model._id, {});
	...
	* save( model ) {
		...
		if (!model._ids)
			...
			yield this._db.collection(model._collectionName).update(model._ids, model._data);
		}
