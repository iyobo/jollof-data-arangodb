...
class JollofDataArangoDB {
	...
	* find( collectionName, criteria, params ) {
		const cursor = yield this._db.collection(collectionName).byExample(criteria, params.opts)
		return cursor.all();
	}

	* update( collectionName, criteria, newValues, params ) {
		const res = yield this._db.collection(collectionName).updateByExample(criteria, newValues, params);
		return res;
	}

	* remove( collectionName, criteria,params ) {
		const res = yield this._db.collection(collectionName).removeByExample(criteria, params);
		return res;
	}
